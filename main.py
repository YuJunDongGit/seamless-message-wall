...
def init_db():
    ...
    if cursor.fetchone() is None:
        admin_password = hashlib.md5(
            os.environ.get('ADMIN_PASSWORD', 'admin').encode()
        ).hexdigest()
...
# 启动时确保数据库存在（gunicorn / WSGI 部署也有效）
with app.app_context():
    init_db()
